...
<?php 
$query="SELECT rmaid,stageid,tracode,leename,codcode,itmname,sn,doa,inwar,bumerang,malfunction,date_format(purchdate,'%d-%c-%Y')AS 'purchdate',purchdoc,transport,transportcomments,contactname FROM rmaservice where rmaid='$rmaid' order by rmaid";
...
$query="SELECT date_format(date,'%d-%c-%Y')AS 'charge_date',date_format(date1,'%d-%c-%Y')AS 'reply_date',value,comment,userid,tiki_user FROM charges where rmaid='$rmaid' and chargeid='$chargeid' order by rmaid";
$result=mysql_query($query);
$num=mysql_numrows($result);
...
for ($i = 0; $i < $num; $i++ )
	...
	$reply_date=mysql_result($result,$i,"reply_date");
	$value=mysql_result($result,$i,"value");
	$comment=mysql_result($result,$i,"comment");
	$userid=mysql_result($result,$i,"userid");
	$reply_user=mysql_result($result,$i,"tiki_user");
	}
...
<tr>
      <td width="20"></td>
      <td width="135"><font color="<?php print $colour1?>">Ημ/νία επιβεβαίωσης</font></td>
      <td width="10"><font color="<?php print $colour1?>">:</font></td>
      <td width="50" colspan="4"><?php  print $reply_date; ?></td>
</tr>
<tr>
      <td width="20"></td>
      <td width="135"><font color="<?php print $colour1?>">Χρήστης επιβεβαίωσης</font></td>
      <td width="10"><font color="<?php print $colour1?>">:</font></td>
      <td width="50" colspan="4"><?php  print $reply_user; ?></td>
</tr>
<?php 
}
?>
