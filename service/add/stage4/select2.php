...
<FORM action="select2a.php"  target="_bottom2" method=post>
<?php 
...
$leename = substr("$_POST[ppp]",strpos($_POST[ppp],$needle)+3,500);
 ...
 #2 εγινε παραλαβη
 #3 εγινε αποστολη accepted only
 #4 γινε αποστολη rejected only                                                               
$query="SELECT * FROM rma.sort_master where traid='$traid' and openflag IN (2,3,4)";
...
for ($i = 0; $i < $num; $i++ )
{
	...
	$dated=mysql_result($result,$i,"date");
	$display_block.="<OPTION value=\"$traid@@@$leename@@@$masterid\">($online) - ($masterid) ---- ($leename) ---- ($label) - ($dated)</OPTION>";
	}
