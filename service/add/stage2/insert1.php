...
<?php
  ...
  if ($error=="0")
  {
	if ($selections){
	 ...
	 $qtya = substr($part2,0,strpos($part2,$needle));
	 $rmaid = substr($part2,strpos($part2,$needle)+1,500);
	 $query = "INSERT INTO transactions VALUES (\"\",\"$rmaid\",\"$date_paralavh\",\"Παραλαβή\",\"$userid\",\"\",\"$docid\",\"$docdetails\",\"\")";
	 $result=mysql_query($query);
	 $query = "INSERT INTO package VALUES (\"\",\"$rmaid\",\"1\",\"1\",\"\")";
	 $result=mysql_query($query);
	 $query = " UPDATE rmaservice set stageid=2 where rmaid = '$rmaid'";
	 $result=mysql_query($query);
	 }
	}
